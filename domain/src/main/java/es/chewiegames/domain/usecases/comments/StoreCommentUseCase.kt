...
class StoreCommentUseCase(val repository: CommentsRepository) : UseCase<Comment, CommentParams>() {

    override fun runInBackground(params: CommentParams): Flow<Comment> =
            repository.storeCommentInDatabase(params.commentText, mapToPostData(params.post))
                    .map {
                        mapToComment(it)
                    }
}
