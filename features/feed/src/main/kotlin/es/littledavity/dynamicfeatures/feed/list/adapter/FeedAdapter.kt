...
import es.littledavity.dynamicfeatures.feed.list.adapter.holders.ErrorViewHolder
import es.littledavity.dynamicfeatures.feed.list.adapter.holders.LoadingViewHolder
import es.littledavity.dynamicfeatures.feed.list.adapter.holders.PostViewHolder
...
class FeedAdapter @Inject constructor(
    ...
    private var state: FeedAdapterState = FeedAdapterState.Added
    ...
    override fun getItemId(position: Int) =
        when (getItemView(position)) {
            ItemView.POST -> getItem(position)?.id?.toLong() ?: -1L
            ItemView.LOADING -> 0L
            ItemView.ERROR -> 1L
        }
     ...
     */
    fun submitState(newState: FeedAdapterState) {
        val oldState = state
        state = newState
