...
import es.littledavity.core.utils.getBitmap
...
//            val width = height / 3
                val cardViewHeight = it.bottom.toFloat() - it.top.toFloat()
                val cardViewWidth = cardViewHeight / 3
                val top = itemView.top.toFloat() + it.top.toFloat()
                val bottom = top + (it.bottom - it.top)
    ...
    class Builder(var dragDirs: Int, var swipeDirs: Int) {
        ...
        fun setCardView(`val`: CardView?): Builder {
            cardView = `val`
            return this
        }
